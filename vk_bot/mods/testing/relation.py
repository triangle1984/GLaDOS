...
class Relation(BasicPlug):
    ...
    def accept(self):
        ...
        if uid2:
            ...
            self.sendmsg("запросов не поступало"); return
    def deny(self):
    ...
    def offer(self):
        ...
        if int(uid2) == self.uid:
            self.sendmsg("низя отправить предложение самому себе"); return
        if self.mc.get(f"r{uid2}"):
            self.sendmsg("вы уже отправили запрос"); return
        elif checktable("relation", "id2 or id", self.uid):
            self.sendmsg("ты уже встречаешься"); return
        else: 
    ...
    def statrelation(self):
        if not checktable("relation", "id2 or id", self.uid):
            self.sendmsg("ни с кем не встречаешься"); return
        else:
